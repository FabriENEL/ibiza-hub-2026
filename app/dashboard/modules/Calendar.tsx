...
export default function Calendar({ hubId, theme, isOwner, archived, persona }: { hubId: string; theme: Theme; isOwner: boolean; archived: boolean; persona: Persona }) {
  ...
  const load = async () => {
    setLoading(true);
    const { data: ev } = await supabase.from('events').select('id, title, description, scheduled_at, location').eq('hub_id', hubId).order('scheduled_at', { ascending: true });
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        {isOwner && !archived && (
          <button onClick={() => setAdding(!adding)} className={'bg-gradient-to-r ' + theme.gradient + ' text-slate-950 text-[10px] px-3 py-1.5 rounded-lg font-black uppercase'}>
      ...
      {events.length === 0 ? (
        <p className="text-slate-500 text-center py-10 text-sm">{isOwner ? w.emptyEvents : 'L organizzatore non ha ancora inserito nulla.'}</p>
      ) : (
              ...
              <div key={ev.id} className={'bg-slate-900 border ' + theme.border + ' p-4 ' + r}>
                <div className="flex gap-4 items-center">
                  ...
                  <div className="border-l border-slate-700 pl-4 flex-1">
                    ...
                    {ev.location && (
                      <button onClick={() => navigateTo(ev.location!)} className={'inline-flex items-center gap-1 mt-1 text-[11px] ' + theme.text + ' hover:underline'}>
                        ...
                        <div key={c.id} className="bg-slate-950 rounded-lg p-2.5 border border-white/5">
                          ...
                          {editing === c.id ? (
                            <div className="flex gap-2 mt-1">
                              <input value={editText} onChange={(e) => setEditText(e.target.value)} className="flex-1 bg-slate-900 border border-slate-700 rounded p-1.5 text-xs text-white outline-none" />
